...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    for (int i = 0; i < sample_count; i++) {
      ...
      Vec3f H = normalize(normalize(V) + normalize(L));
      float NdotL = std::max(0.f, dot(N, normalize(L)));

      float F = 1.f;  // 计算出射的总能量，因此Fresnel term取1
      float D = DistributionGGX(N, H, roughness);
      float G = GeometrySmith(roughness, NdotV, NdotL);

      float fr =  F * G * D / std::max(4.f * NdotV * NdotL, .001f);
      A = B = C += fr * NdotL / pdf;
    }
...
int main() {
    ...
    for (int i = 0; i < resolution; i++) {
        for (int j = 0; j < resolution; j++) {
            ...
            float NdotV = step * (static_cast<float>(j) + 0.5f);  // 法线N = (0., 0., 1.)
