        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Vector3f dir = cubemapDirs[i * width * height + y * width + x];  // 环境光的方向
                    int index = (y * width + x) * channel;
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);

                    // 将法线方向转为球坐标(theta phi), ToSphericalCoords dir参数类型为Eigen::Vector3d,
                    // 且Vector3f和Vector3d不能直接转换因此需要用temp作为中间变量
                    Eigen::Vector3d temp = {dir.x(), dir.y(), dir.z()};
                    double theta, phi;
                    sh::ToSphericalCoords(temp.normalized(), &phi, &theta);

                    // 计算一个cubemap上一个pixel投影到球面的面积
                    float pixelArea = CalcArea((float)y, (float)x, width, height);

                    // 计算Lighting对应的球谐函数系数, 6个cubemap面对应的球谐函数系数相加
                    for(int l = 0; l <= (int)SHOrder; l++)
                    {
                        for(int m = -l; m <= l; m++)
                        {
                            int sh_index = sh::GetIndex(l, m);  // 计算sh的index
                            // 计算sh_index对应的球谐函数在phi theta处的值
                            double sh_value = sh::EvalSH(l, m, phi, theta);
                            SHCoeffiecents[sh_index] += sh_value * pixelArea * Le;
                        }
                    }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            const Normal3f &n = mesh->getVertexNormals().col(i);  // n是shading point的法线方向

            auto shFunc = [&](double phi, double theta) -> double {
                Eigen::Array3d d = sh::ToVector(phi, theta);
                // wi是环境光的入射方向的反方向, wi是通过在shading point半球随机采样生成的
                const auto wi = Vector3f(d.x(), d.y(), d.z()).normalized();
                if (m_Type == Type::Unshadowed)
                {
                    // TODO: here you need to calculate unshadowed transport term of a given direction
                    // TODO: 此处你需要计算给定方向下的unshadowed传输项球谐函数值
                    double H = wi.dot(n.normalized());  // cosine term
                    return H > 0. ? H : 0.;
                }
                else
                {
                    ...
                    Ray3f ray(v, wi);
                    double H = wi.dot(n.normalized());  // cosine term
                    if(H > 0. && !scene->rayIntersect(ray))  // 从shading point v向wi发射的该光线没有与场景相交
                        return H;
                    return 0.;
                }
            };
            ...
            {
                m_TransportSHCoeffs.col(i).coeffRef(j) = (float)(*shCoeff)[j] * INV_PI;
            }
        }
        if (m_Type == Type::Interreflection)
        {
            ...
            for(unsigned i = 0; i < mesh->getVertexCount(); i++)
            {
                ...
                auto vertexIndirCoeffs = ComputeIndirCoeffs(m_TransportSHCoeffs, v, n, scene, mesh, 1);

                for(int j = 0; j < SHCoeffLength; j++)
                {
                    m_TransportSHCoeffs.col(i).coeffRef(j) += (float)(*vertexIndirCoeffs)[j] * INV_PI;
                }
            }
        }
...
//        }
     ...
     */
    std::unique_ptr<std::vector<double>> ComputeIndirCoeffs(MatrixXf& DirLTCoeffs, const Point3f& point,
                                                            const Normal3f& normal, const Scene* scene,
                                                            Mesh* const mesh, int bounceTime)
    {
        std::unique_ptr<std::vector<double>> InterpolatedBarySH =
                std::make_unique<std::vector<double>>(SHCoeffLength, 0.);

        if(bounceTime > m_Bounce)
            return InterpolatedBarySH;
        ...
        for(int t = 0; t < sample_side; t++)
        {
            for(int p = 0; p < sample_side; p++)
            {
                ...
                Eigen::Array3d d = sh::ToVector(phi, theta);
                Vector3f wi = Vector3f(d.x(), d.y(), d.z()).normalized();
                Ray3f ray(point, wi);
                Intersection its;
                bool isIts = scene->rayIntersect(ray, its);

                if(!isIts)
                    continue;

                double H = wi.dot(normal);  // cosine term
                if(H <= 0.f)
                    continue;

                Vector3f baryCoords = its.bary;
                // Compute barycenter Normal
                Normal3f baryNormal = (mesh->getVertexNormals().col(its.tri_index.x()) * baryCoords.x() +
                                       mesh->getVertexNormals().col(its.tri_index.y()) * baryCoords.y() +
                                       mesh->getVertexNormals().col(its.tri_index.z()) * baryCoords.z()).normalized();

                // Recursive compute Indir SHCoeffs in intersection point
                auto nextBounce = ComputeIndirCoeffs(DirLTCoeffs, its.p, baryNormal, scene, mesh, bounceTime+1);

                // BaryCentric Interpolation for SH, take cosine term into consideration
                // This transfer function is the amount of light being reflected back towards your shading point.
                for(int shNum = 0; shNum < SHCoeffLength; shNum++)
                {
                    (*InterpolatedBarySH)[shNum] += H *
                            (
                                    DirLTCoeffs.col(its.tri_index.x()).coeff(shNum) * baryCoords.x() +
                                    DirLTCoeffs.col(its.tri_index.y()).coeff(shNum) * baryCoords.y() +
                                    DirLTCoeffs.col(its.tri_index.z()).coeff(shNum) * baryCoords.z() +
                                    (*nextBounce)[shNum]  // The SH of the barycenter point
                            );
                }
            }
        }
        ...
        for (unsigned int i = 0; i < SHCoeffLength; i++) {
            (*InterpolatedBarySH)[i] *= weight;
        }
        return InterpolatedBarySH;
    }
